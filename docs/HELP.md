...
評価では Jev が適切さ、重要情報、簡潔性、正確性、合否を判定し、Groq が改善点を3件示します。総合得点は3つの5段階評価の元値を等重みで0〜100点に換算します。どちらかのサービスが失敗した評価は統計に保存されません。
...
文章生成には Groq キー、要約評価には Jev キーが必要です。メニューの `s` で設定画面を開き、両方のキーを個別に保存できます。保存済みキーの実値は画面に表示しません。

トレーニング画面からも `s` で設定画面を開けます。Groq キーを入力して `Enter` を押すと Jev キー欄へ移り、もう一度 `Enter` を押すと保存します。空欄は現在の値を維持します。`Esc` は保存せず元の画面へ戻ります。
...
2. **環境変数**:
   ...
   `GROQ_API_KEY` は設定ファイル内の Groq キーより優先されます。Jev キーは設定画面または設定ファイルで設定してください。
...
- **180日レポート**: 過去 180 日間のトレーニング結果を、週横軸・曜日縦軸のヒートマップで表示
- **週次レポート**: 過去 4 週間の週別結果
- **成功率**: 正解率の推移
- **トレーニング回数**: 総回数と正解/不正解の内訳
- **評価スコア**: 直近 180 日の平均・中央値・件数
