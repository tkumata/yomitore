...
- アプリケーション起動時に、以下の優先順位で GroqCloud API キーを取得する：
...
- API キーは環境変数、または利用者が作成した暗号化されていない設定ファイルから読み込む
- Unix 系で設定ファイルを使用する場合、利用者が 600 パーミッション（所有者のみ読み書き可能）に設定する
- 環境変数からの読み込みをサポート
...
- 実際の GroqCloud API を利用せずに評価結果の解析と合否判定を検証できること
