...
- **プロンプト**: "日本の公的文書および新聞記事に共通する、客観的で簡潔かつ形式的な文体で、感情的表現や口語表現を避けた文章をN文字程度で生成してください。" (推論能力のないLLMの性能向上のため、このプロンプトを2回繰り返して送信する場合があるが、現在のコードでは1回のみ記述されている)
- API タイムアウト: 60 秒
- 生成された文章を画面左半分に表示する
...
- ユーザーの入力送信後、以下の情報を GroqCloud API に送信する：
  - **原文**: アプリケーションが生成した文章
  - **要約文**: ユーザーが入力した文章
- **評価プロンプト**: 「はい」または「いいえ」で判定し、5 段階評価を含む詳細なフィードバックを提供（推論能力のないLLMの性能向上のため、プロンプト全文を2回繰り返して送信する）
